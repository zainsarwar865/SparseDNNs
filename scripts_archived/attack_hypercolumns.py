...
if args.integrated:
    if args.detector_type == 'Quantized':
        from torchattacks.attacks.cw_integrated_quantized_hypercolumns import CW_MLP as CW
    elif args.detector_type == 'Regular':
        from torchattacks.attacks.cw_integrated_hypercolumns import CW_MLP as CW
else:
...
yaml_file = os.path.join(expr_dir, f"Logs/Attack_Config_integrated-{args.integrated}_Type-{args.detector_type}.yaml")
...
for i in range(args.batch_size):
    mask[i, torch.randperm(1024)[:num_dimensions]] = True


if args.integrated:
    if args.attack == "CW":
        atk = CW(model, mlp, mask, c=args.c_attack, d=args.d_attack, steps=args.steps, lr=args.lr)
else:     
    if args.attack == "CW":
