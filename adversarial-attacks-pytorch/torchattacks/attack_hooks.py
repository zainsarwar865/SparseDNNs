...
class Attack(object):
    ...
    def __init__(self, name, model):
        ...
        self.avgpool_hook = model.avgpool.register_backward_hook(self.avg_hook)
    ...
    def avg_hook(self, module, grad_in, grad_out):
        grad = grad_in[0].squeeze()
        if isinstance(self.pool_grads, torch.Tensor):
            torch.concatenate((self.pool_grads, grad), dim=0)
        else:
            self.pool_grads = grad
