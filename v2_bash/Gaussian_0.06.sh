...
seed=42
attack="CW"
#epsilon_list=(0.002 0.006 0.01 0.014 0.018 0.022 0.026 0.03 0.034 0.038 0.044 0.048 0.052 0.056 0.06 0.064 0.070 0.090 0.150 0.2 0.5)
epsilon_list=(0.075 0.085 0.95 1.05 1.25)
epsilon=0.0
#epsilon_list=(0.038 0.04)
#epsilon=0
attack_type="Bounded"
c_base=1.0
d_base=0.0
...
model='resnet18_Gaussian'

#############################################################################
if [ "$model" = 'resnet18_randCNN' ] || [ "$model" = 'resnet18_sharded' ] || [ "$model" = 'resnet18_randCNN_l2' ]
then    
    scale_factor=5
...
if [ "$model" = 'resnet18_Gaussian' ] || [ "$model" = 'resnet18_Gaussian_Weights' ]
then    
    gaussian_dev=0.06
fi
...
mt_hash_config="${trainer_type}_${original_dataset}_${original_config}_${model}_pretrained-${pretrained}_lr-${lr}_batch_size-${batch_size}_eval-pretrained-${eval_pretrained}_scale-factor_${scale_factor}_sparseblock-${sparseblock}_gaussian-dev_${gaussian_dev}seed-${seed}"
...
then
    ...
    then
        ...
        if [[ "$attack_type" == "Unbounded" ]]
        then
                cd ${home_dir}
                python3 attack_unbounded.py \
                --gpu=$gpu \
                --base_dir=$base_dir \
                --root_hash_config=$root_hash_config \
                --mt_hash_config=$mt_hash_config \
