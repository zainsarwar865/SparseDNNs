            ...
            for i, (images, target) in enumerate(loader):
                ...
                for k in output.keys():
                    if k not in features.keys():
                        ...
                        features[k] = torch.concat((features[k], output[k]), dim=0)
            
                # Save dict here
                with open(relu_dict_path, "w") as out_file:
...
relu_dict = f"ConvFeatures_{args.attack}_{args.extract_split}_{args.extract_type}_{args.total_attack_samples}_detector-type-{args.detector_type}_integrated-{args.integrated}_c-{args.c}_d-{args.d}.pth"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Test after training
    if args.extract_split == 'train':
        validate_per_class(train_loader, model, args)
    elif args.extract_split == 'test':
