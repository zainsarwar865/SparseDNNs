...
if args.sparsefilter == 'SparsifyFiltersLayer':
    ...
    from utils.resnet_rand_tracer_1 import resnet18, SparsifyFiltersLayer, SparsifyKernelGroups
...
if args.sparsefilter == 'SparsifyFiltersLayer':
    sparseblock = SparsifyFiltersLayer
    from utils.resnet_rand_tracer_2 import resnet18, SparsifyFiltersLayer, SparsifyKernelGroups
elif args.sparsefilter == 'SparsifyKernelGroups':
    from utils.resnet_rand_tracer_2 import resnet18, SparsifyFiltersLayer, SparsifyKernelGroups
    sparseblock = SparsifyKernelGroups
elif args.sparsefilter == "None":
...
def validate_per_class(val_loader, model, args):
    def run_validate(loader, base_progress=0):
        ...
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                # compute output
                output = model(images)
                # Concat all features to the appropriate layer
                for k in output.keys():
                    if k not in features.keys():
                        features[k] = output[k].detach().cpu()
                    else:
                        features[k] = torch.concat((features[k], output[k].detach().cpu()), dim=0)
            
            # Save dict here
            with open(relu_dict_path, "wb") as out_file:
                pickle.dump(features, out_file)
        return features
    # switch to evaluate mode
    model.eval()
    features = run_validate(val_loader)
    return features
...
relu_dict = f"ConvFeatures_{args.attack}_{args.attack_split}_{args.test_type}_{args.total_attack_samples}_detector-type-{args.detector_type}_integrated-{args.integrated}_c-{args.c}_d-{args.d}_3:4.pth"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if args.arch == 'wideresnet':
            ...
            if args.sparsefilter == "None":
                ...
                print("Loading the randomresnet model.")
        elif args.arch == 'MLP':
            model = MLP()
    ...
    if args.test_type == 'adversarial':
        ...
        flipped_indices = [7,8,9,10,11,12,13,14]
    ...
    model = ResNetFeatureExtractor(model=model)

    # Extract features for test data
    validate_per_class(val_loader, model, args)
