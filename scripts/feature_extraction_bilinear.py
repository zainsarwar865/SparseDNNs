...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.extract_split == 'train':
        ...
        extract_features(val_loader, model, args)
    ...
    def run_validate(loader, base_progress=0):
        ...
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                # Concat features
                if isinstance(features_x6, torch.Tensor):
                    features_x6 = torch.concat((features_x6, x6.detach().cpu()),dim=0)
                else:
                    features_x6 = x6.detach().cpu()
                
                if isinstance(features_x4, torch.Tensor):
                    features_x4 = torch.concat((features_x4, x4.detach().cpu()),dim=0)
                else:
                    features_x4 = x4.detach().cpu()

                if isinstance(features_x3, torch.Tensor):
                    features_x3 = torch.concat((features_x3, x3.detach().cpu()),dim=0)
                else:
                    features_x3 = x3.detach().cpu()

                if isinstance(features_x1, torch.Tensor):
                    features_x1 = torch.concat((features_x1, x1.detach().cpu()),dim=0)
                else:
                    features_x1 = x1.detach().cpu()

                if isinstance(features_x0, torch.Tensor):
                    features_x0 = torch.concat((features_x0, x0.detach().cpu()),dim=0)
                else:
                    features_x0 = x0.detach().cpu()


            features_x6 = features_x6.unsqueeze(-1).unsqueeze(-1)
            #x6 = x6.unsqueeze(-1).unsqueeze(-1)
            up_size = (32, 32)
            upsample_layer = torch.nn.Upsample(size=up_size, mode='bilinear')
            features_x6 = upsample_layer(features_x6)
            features_x4 = upsample_layer(features_x4)
            features_x3 = upsample_layer(features_x3)
            features_x1 = upsample_layer(features_x1)
            features_x0 = upsample_layer(features_x0)
            # Concat across filter dimension and reshape
            features = torch.concat((features_x6, features_x4, features_x3, features_x1, features_x0), dim=1)
            features = features.flatten(2,3)
            features = features.transpose(1,2)        
            #Randomly select N dimensions per data point
            num_dimensions = 200
            mask = torch.zeros(features.shape[0], features.shape[1], dtype=bool) # (batch_size, image_size)

            for i in range(features.shape[0]):
                mask[i, torch.randperm(features.shape[1])[:num_dimensions]] = True
            # Subselect using boolean mask
            subselected_features = features[mask]
            # Reshape to proper dimensions
            features = subselected_features.view(features.shape[0], num_dimensions, features.shape[2])
