...
class SparsifyKernelGroups(nn.Module):
    ...
    def forward(self, conv_filters):
        ...
        """
        conv_filters = conv_filters[:, 1::4,  :, : ]
        #new_filters = conv_filters.shape[1] // self.div_factor
        #conv_filters = conv_filters[:, 0:new_filters,  :, : ]
        return conv_filters
...
class SparsifyFiltersLayer(nn.Module):
    ...
    def forward(self, conv_filters):
        tot_weights = conv_filters.shape[0] * conv_filters.shape[2] * conv_filters.shape[3]
        new_filters = conv_filters.shape[1] // self.div_factor
        weights = torch.ones(size=(tot_weights, conv_filters.shape[1]))
        indices = torch.multinomial(input=weights, num_samples=new_filters, replacement=False)
        indices = indices.reshape(conv_filters.shape[0],conv_filters.shape[2], conv_filters.shape[2], new_filters )
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv3x3(inplanes, planes*self.scale_factor, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes*self.scale_factor)
