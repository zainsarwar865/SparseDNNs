  ...
  def forward(self, x):
    ...
    for idx, (name, param) in enumerate(self.model.named_parameters()):
        if param.requires_grad and "conv" in name:
            ...
            noise = torch.randn(param.data.size()).to(device=param.device) * self.std
            # Add noise to the weights
            name_lst = name.split(".")[:-1]
            if idx == 0:
                self.model._modules[name_lst[0]].weight.data+= noise
            else:
                getattr(self.model._modules[name_lst[0]][int(name_lst[1])], name_lst[2]).weight.data+=noise
    ...
    for idx, (name, param) in enumerate(self.model.named_parameters()):
        if param.requires_grad and "conv" in name:
            name_lst = name.split(".")[:-1]
            if idx == 0:
                self.model._modules[name_lst[0]].weight.data = og_weights[name]
            else:
                getattr(self.model._modules[name_lst[0]][int(name_lst[1])], name_lst[2]).weight.data = og_weights[name]
